...
import Page from "./page_factory.js";
...
function SimulationWindow() {
  ...
  async function getPagesData() {
    const response = await fetch(BASE_URL + `/pages/?student_id=1&scenario_id=${sid}`, {
      "method": "GET",
      "credentials": "include"
    });
  ...
  useEffect(() => {
    getPagesData().then(pagesData => {
      const pageData = pagesData.results.slice();

      let pageComponents = {};
      
      pageData.forEach((page) => {
          pageComponents[page.page] = 
            new Page(false, false, page.page, page.page_type, 3, page.page_title, page.body)
      });
